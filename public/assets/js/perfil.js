...
async function carregarPerfil() {
  ...
  try {
    const resposta = await fetch(`http://0.0.0.0:10000/usuarios/${usuarioId}`);
...
document.getElementById('salvar-btn').addEventListener('click', async () => {
  ...
  try {
    await fetch(`http://0.0.0.0:10000/usuarios/${usuarioId}`, {
      method: 'PUT',
...
async function carregarDenuncias() {
  const resposta = await fetch('http://0.0.0.0:10000/denuncias');
...
async function excluirDenuncia(id) {
  if (confirm('Tem certeza que deseja excluir esta denúncia?')) {
    await fetch(`http://0.0.0.0:10000/denuncias/${id}`, { method: 'DELETE' });
    carregarDenuncias();
  }
}
...
document.getElementById('excluir-btn').addEventListener('click', async () => {
  if (confirm('Tem certeza que deseja excluir sua conta? Esta ação é irreversível.')) {
    await fetch(`http://0.0.0.0:10000/usuarios/${usuarioId}`, { method: 'DELETE' });
    localStorage.clear();
